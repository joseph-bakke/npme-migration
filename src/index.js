...
async function publishToNpm({ tarballPath, distTag }) {
    let command = ['npm', 'publish', tarballPath];

    if (distTag) {
        command = [...command, '--tag', distTag];
    }
...
async function migratePackages() {
    ...
    const { doc_count: initialZnpmDocCount } = await getRegistryMetaInfo(ZNPM_URL);
    ...
    await Promise.each(unpublishedVersions, transformTarball);
